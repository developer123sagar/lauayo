...
import { GET_CONTENT, Post_ContentPolicy } from "@/constants/API";
...
export const EditContentpolicy = () => {
  ...
  } = useGetApiQuery({ api: GET_CONTENT });

  useEffect(() => {
    if (!isGetDataLoading && !isError && data) {
      // Initialize the form state with data fetched from the API
      setForm({
        title: data.title,
        body: data.body,
      });
